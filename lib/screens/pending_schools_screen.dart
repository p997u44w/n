...
class _PendingSchoolsScreenState extends State<PendingSchoolsScreen> {
  ...
  Future<void> _reject(int schoolId) async {
    final res = await ApiService.post('admin/reject-school', {'school_id': schoolId});
              ...
              onPressed: () async {
                ...
                if (res['success'] == true) {
                  ...
                  ScaffoldMessenger.of(this.context).showSnackBar(
                    const SnackBar(content: Text('مدرسه با موفقیت تایید شد')),
                  );
                } else {
                  // دیالوگ را نبند؛ خطای واقعی API را به ادمین نشان بده.
                  ScaffoldMessenger.of(this.context).showSnackBar(
                    SnackBar(content: Text(res['message']?.toString() ?? 'تایید مدرسه انجام نشد')),
                  );
                }
              },
